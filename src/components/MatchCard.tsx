      ...
      <div className="px-3 sm:px-4 py-3 sm:py-4">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
            <span className="text-2xl sm:text-3xl flex-shrink-0">{match.homeTeam.logo}</span>
            <div className="min-w-0">
              <p className="font-bold text-white text-xs sm:text-sm truncate">{match.homeTeam.name}</p>
              {match.status === 'live' && (
                <p className="text-xl sm:text-2xl font-bold text-white">{match.homeScore}</p>
              )}
            </div>
          </div>
          <div className="px-2 sm:px-4 flex-shrink-0">
            <span className="text-gray-600 font-bold text-base sm:text-lg">VS</span>
          </div>
          <div className="flex items-center gap-2 sm:gap-3 flex-1 justify-end text-right min-w-0">
            <div className="min-w-0">
              <p className="font-bold text-white text-xs sm:text-sm truncate">{match.awayTeam.name}</p>
              {match.status === 'live' && (
                <p className="text-xl sm:text-2xl font-bold text-white">{match.awayScore}</p>
              )}
            </div>
            <span className="text-2xl sm:text-3xl flex-shrink-0">{match.awayTeam.logo}</span>
          </div>
        </div>
        ...
        <div className="mb-3">
          ...
          <div className="flex h-7 sm:h-8 rounded-lg overflow-hidden border border-grafite-500">
            <div
            ...
            >
              <span className="text-[9px] sm:text-[10px] font-bold">E {match.drawProb}%</span>
            </div>
            <div
        ...
        {match.valueBet && (
          <button className="w-full py-3 rounded-xl bg-gradient-to-r from-gold/20 to-gold/10 border border-gold/40 flex items-center justify-center gap-1.5 sm:gap-2 group-hover:from-gold/30 group-hover:to-gold/20 transition-all flex-wrap">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[10px] sm:text-xs font-bold text-gold">APOSTA DE VALOR</span>
            </div>
